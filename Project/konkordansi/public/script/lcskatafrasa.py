...
def preprocessQueryWoStopword(query):
    preQuery = query.lower()
    # preQuery = re.sub(r'[^a-zA-Z0-9 ]',r'',preQuery)
    # preQuery = preQuery.translate(None, string.punctuation) 
    porter_stemmer = PorterStemmer()
    ...
    hasilPemotongan = []
    for w in word_tokens:
        hasilPemotongan.append(porter_stemmer.stem(w))
...
# membuat query
if len(preQuery) == 0:
    arrQuery = preprocessQueryWoStopword(query)
    for x in arrQuery:
        if z == 0:
...
for row in cur.fetchall():
    ...
    for y in splitt:
        ...
        if query in x:
            ...
            pos         = x.find(query)
            marked      = x[:pos] + '<b>' + x[pos:pos+int(len(query))] + '</b>' + x[pos+int(len(query)):]

            findStr     = source.find(query)
            joinedStr   = source[:findStr] + marked + source[findStr+int(len(query)):]
            lastStr     = int(findStr) + int(len(marked))
            
            arr.append([row[0], row[1], row[2], row[6], row[7], row[8], query, joinedStr, percent, int(findStr), lastStr])            
        else:    
            ...
            if (len(check) > 0):
                ...
                if (percent>=lower and percent<=upper):
                    ...
                    findStr     = source.find(y)
                    joinedStr   = source[:findStr] + marked + source[findStr+int(len(x)):]
                    lastStr     = int(findStr) + int(len(marked))

                    # print repr(callLcs) + repr(percent)+ " Indeks => " + repr(row[1]) + ":" + repr(row[2])
                    arr.append([row[0], row[1], row[2], row[6], row[7], row[8], callLcs, joinedStr, percent, int(findStr), lastStr])
